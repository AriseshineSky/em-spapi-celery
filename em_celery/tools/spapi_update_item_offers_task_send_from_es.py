...
def send_spapi_item_offers_update_task(broker_url, index, marketplace='us', condition='new', ttl=36, force=False):
  configure_sender('em_celery.tools.spapi_update_item_offers_task_send_from_es',
                   'spapi_update_item_offers_task_sender.log')
  broker_url = normalize_broker(broker_url)

  sender = SpapiUpdateItemOffersTaskSender(broker_url, marketplace, condition, index, ttl, force)
  sender.run()
...
class SpapiUpdateItemOffersTaskSender():
  ...
  def __init__(self, broker_url, marketplace, condition, index, ttl, force):
    ...
    self.asin_indice_name = f'amz_asins_{self.marketplace}_no_offer'
    if index:
  ...
  def run(self):
    ...
    batch_size = 500 
    while True:
        for s, _ in self.product_service.load_products_by_after_search(self.asin_indice_name):
