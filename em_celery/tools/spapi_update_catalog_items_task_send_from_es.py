...
@click.option('-i', '--index', type=str, default=None, help='Amazon marketplace to fetch catalog items.')
...
def send_spapi_catalog_items_update_task(broker_url, qps, index, marketplace='us', ttl=168, force=False):
  configure_sender('em_celery.tools.spapi_update_catalog_items_task_send_from_es',
                   'spapi_update_catalog_items_task_sender.log')
  broker_url = normalize_broker(broker_url)

  sender = SpapiUpdateCatalogItemsTaskSender(broker_url, qps, marketplace.lower(), index, ttl, force)
  sender.run()
...
class SpapiUpdateCatalogItemsTaskSender():
  ...
  def __init__(self, broker_url, qps, marketplace, index, ttl, force):
    ...
    self.asin_indice_name = 'amz_asins_{}_no_info'.format(self.marketplace)
    if index:
        self.asin_indice_name = index
  ...
  def run(self):
    ...
    batch_size = 500
    time_key = "timestamp"
    if "keyword" in self.asin_indice_name:
        time_key = "time"

    while True:
        for s, _ in self.product_service.load_products_by_after_search(self.asin_indice_name, "1999-01-01T00:00:01.722593+00:00", time_key):
  ...
  def process_products(self, asins):
    ...
    for chunk in chunks:
      spapi_update_catalog_items.apply_async(
          args=(self.marketplace, chunk), queue=self.queue, connection=self.connection
      )
      logger.debug(
