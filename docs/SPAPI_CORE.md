...
**Q：Rate limit 是多少？**  
A：Celery task 注解：`spapi_update_item_offers` **6/m**，`spapi_update_catalog_items` **2/s**（均为**每个 Worker 实例**独立 TokenBucket）。Amazon 429 由 Layer 1–3 重试处理。多机如何叠加，见 [SPAPI_RATE_LIMITING.md](./SPAPI_RATE_LIMITING.md)。
