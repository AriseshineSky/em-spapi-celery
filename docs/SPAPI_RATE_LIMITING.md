...
假设 **1 个 seller 凭证 + 1 个 app**，生产示例 `CELERY_OFFER_CONCURRENCY=4`、`CELERY_CATALOG_CONCURRENCY=2`，单机部署：

| API | 官方持续上限 | 本项目 Layer 0 峰值（单机） | 是否对齐 |
|-----|-------------|---------------------------|----------|
| Offer batch | **6/min**（0.1/s） | 4 × 8/min = **32/min** | **否**，约 **5.3×** |
| Offer burst | **1** 并发 token | 最多 **4** 子进程同时调 API | **否** |
| Catalog | **2/s**（120/min） | 2 × 1/s = **2/s** | **持续速率接近** |
| Catalog burst | **2** | 2 子进程可同时各 1 次 | **边界**（桶满时 OK，多机叠加会超） |

结论：**Offer 是当前主要超标风险**；Catalog 在单机 concurrency=2 时尚可，多机或多副本需按 §8.4 反算。
...
flowchart TB
  ...
  subgraph Celery["Celery Worker（prefork）"]
    RL["Layer 0: rate_limit<br/>offer 8/m · catalog 1/s<br/>每子进程独立"]
...
- **`8/m`**：该 **Worker 子进程** 每分钟最多 **开始执行** 8 次 `spapi_update_item_offers`
...
入队 task **默认 priority 0（bulk）**；高优 `:9` 由单独发送路径指定，见 [PRIORITY_QUEUE.md](./PRIORITY_QUEUE.md)。
...
|----|---------------------------|----------|--------------|
| concurrency | 1 | offer 4 / catalog 2 | offer 应 **1**；catalog 单机 **≤2** |
| rate_limit | 8/m、1/s | 同上 | offer 应 **≤6/m**（Σ=1 时） |
